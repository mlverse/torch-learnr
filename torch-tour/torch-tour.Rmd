...
Sys.setenv(TORCH_HOME=path.expand("libtorch/"))
cat("Torch tour logs\n")
print(list.files(Sys.getenv("TORCH_HOME")))
library(learnr)
library(torch)
library(torchvision)
library(ggplot2)
...
set.seed(1)
try(print(torch:::.globals$lantern_started))
try(print(torch:::lantern_start()))
print(torch:::install_path())
print(list.files(torch:::install_path()))
torch_manual_seed(1)
...
3.  We must manually erase the gradients that we just used to update the
    ...
    accumulates gradients after backward, and in general we don't want
...
happens at that time. The returned object is what we call a 'factory' of
